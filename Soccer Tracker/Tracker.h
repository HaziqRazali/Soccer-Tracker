...
namespace st {
	...
	public:
		...
		TrackInfo getTrackInfo() {
			...
			{
				trackInfo.set(-1, Rect(), Point(-1, -1), givenTrajectory[curFrame + 2], metric.results);
			}

			else
			{
				trackInfo.set(mainCandidate->id, mainCandidate->curRect, mainCandidate->curCrd, givenTrajectory[curFrame + 2], metric.results);
			}
		...
		//=========================================================================================
		bool ballFoundAtSideline(Point searchZone, Mat& frame, int TID) {

			// Task -> Add constraints to standard search so that it does not search random player
			for (unsigned i = 0; i < pCandidates.size(); i++)
			{
				Point pFeet(pCandidates[i]->curRect.br());
				Point pHead(pCandidates[i]->curRect.tl().x + pCandidates[i]->curRect.width / 2, pCandidates[i]->curRect.tl().y);
				Rect  pRect(pCandidates[i]->curRect);

				// If player is behind sideline and is not a referee and is near location of last known position
				if ((pFeet.y > 510) && (pFeet.x < 480) && (pCandidates[i]->teamID != 2) && (norm(Mat(pHead), Mat(searchZone), NORM_L2) < 300) && (pCandidates[i]->curRect.area() > 900))
				{
					//Add new candidate and attach it to player head
					BallCandidate* bc = new BallCandidate(curFrame, pHead, iniRad, 0.0);
		...
		void trackBall(Mat& frame, vector<Point>& newCandidates, vector<ProjCandidate*> truePositives, int TID, int processedFrames) {
			...
			// 2 second counter to initialize tracker after ball goes out of play
			//if (ballOutOfPlay > 2) { ballOutOfPlay--; return; }
			...
			switch (trackerState) {
				...
				case TRACKER_STATE::BALL_NOT_FOUND:
				{
					...
					//{
					//	Point lastBallLocation = *(mainCandidateTraj.end() - 1);
					//	
					//	// Activate 2 second counter if ball goes out of play
					//	if (lastBallLocation.y > 500 && ballOutOfPlay == 0)	ballOutOfPlay = 10; // 10 and 70

					//	// Timer up... Task -> Create another time to force out of sideline search
					//	if (ballOutOfPlay == 2) {
					//		if (ballFoundAtSideline(lastBallLocation, frame, TID)) { ballOutOfPlay--; goto done; }
					//		else return;
					//	}
					//}
			...
			case TRACKER_STATE::BALL_FOUND:
			{
				if (ballOutOfPlay == 1) {
					//waitKey(0);
					ball_updateBallCandidate(bCandidates[0], frame, TID, ballOutOfPlay);
					return;
				}
				...
				if (mainCandidate == NULL)	trackerState = TRACKER_STATE::BALL_NOT_FOUND;
				else						mainCandidateTraj.push_back(mainCandidate->curCrd);
		...
		//=========================================================================================
		void ball_updateBallCandidate(BallCandidate* bc, Mat& frame, int TID, int ballOutOfPlay = 0) {
			
			//if (ballOutOfPlay == 1)
			//{
			//	// Find candidate that has the ball Task -> Function will not execute when ball out of player box
			//	for (auto pc : pCandidates) if (pc->curRect.contains(bc->curCrd))
			//	{
			//		// Default position if ball not found
			...
			//			//if (bc->curCrd.y < 450) this->ballOutOfPlay--; // NEWLY ADDED
			...
			switch (bc->getState()) 
			{
				...
				case BALL_STATE::TRACKING: {
					...
					bc->curCrd = nCrd;
					bc->curAppearM = nProb;
					//this->ballOutOfPlay = 0; // Reset constraint Task -> REMOVE as system may track random object
					break;
				}
		...
		void drawTrackingMarks(Mat& frame, BallCandidate* bc, int TID) {
			...
			*********************************************************************************/
			...
			float height = 0;
			for (int i = 0; i < Ball.size(); i++)	if (TID == Ball[i]->cameraID && Ball[i]->coords3D.size() > 0)	height = (Ball[i]->coords3D.end() - 1)->z;
			...
			// Display text
			char label[40];
			sprintf(label, "%f", height/*trackRatio, bc->getStateDuration(BALL_STATE::TRACKING)*//*, bc->id, Ball.size()*//*, bc->curAppearM, Ball.size()*/);
